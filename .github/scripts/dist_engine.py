...
def submit_indexnow(urls):
    payload = json.dumps({"host": "tech-tools-hub.netlify.app", "key": INDEXNOW_KEY,
...
def post_to_bluesky(title, desc, url):
    """Post to Bluesky with clickable link card via AT Protocol facets."""
    ...
    try:
        ...
        # Calculate byte offsets for the URL facet
        url_bytes = url.encode("utf-8")
        text_bytes = text.encode("utf-8")
        url_start = text_bytes.find(url_bytes)
        url_end = url_start + len(url_bytes) if url_start >= 0 else -1
        ...
        # Add facet to make URL clickable
        if url_start >= 0:
            record["facets"] = [{
        ...
        print(f"   \u2705 Bluesky: posted (with clickable link)")
